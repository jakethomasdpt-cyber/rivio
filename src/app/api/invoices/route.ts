...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: invoices, error } = await dbClient
      .from('invoices')
      .select(
        `
        id,
        ...
        paid_date,
        viewed_at,
        veda_organization_id,
        veda_patient_id,
        veda_invoice_id,
        latest_payment_failure,
        notes,
        tax_rate,
        reminder_enabled,
        clients(name, email)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!client_id) {
      ...
      if (!client_id) {
        const { data: newClient, error: clientError } = await dbClient
          .from('clients')
    ...
    const { data: clientExists } = await dbClient
      .from('clients')
      .select('id')
      .eq('id', client_id)
      .eq('user_id', user.id)
